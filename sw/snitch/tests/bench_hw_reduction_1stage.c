...
// system. It can reduce double data directly in fligth with the internal DMA.
// Is is possible to set the target cluster freely.
...
int main() {
    ...
    for(volatile int i = 0; i < 3; i++){
        ...
        if (snrt_is_dm_core() && cluster_participates_in_reduction(cluster_id)) {
            snrt_dma_start_1d_collective(buffer_target, buffer_src, DATA_LENGTH * sizeof(double), (void *) mask, SNRT_COLL_WIDE_FPADD);
